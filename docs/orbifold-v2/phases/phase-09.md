...
- **OQ-2 → Option D (variant D1)** [resolved at Checkpoint #1, 2026-06-12]. Reuse the existing `CodeDrawer.svelte` / `CompositionDrawer.svelte` content; replace the `position:fixed` + `translateY(105%)` slide CSS with a flex-fill layout inside `#stage`; gate each by `{#if $sessionStore.view === ...}` in `App.svelte`; remove the `#codeTab` / `#compTab` drawer tab buttons. ~70-80 lines, full-canvas, low regression risk (Dev + Planner recommendation; E disqualified for ~1100-line duplication, F disqualified for inability to reach full-canvas). Note: under the `{#if}` gate the component is destroyed/remounted on view switch, so CompositionDrawer's rAF playhead loop restarts cleanly on mount — confirmed safe by the early-return guard.
