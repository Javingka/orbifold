...
**Vision:** Enable Orbifold to *interpret real music* — given a song (audio file / YouTube / Spotify link — input set being refined), extract its rhythm and harmony and build a Session that represents it in Orbifold's musical language and visual interface. This is a staged effort: (1) `song-import` Phase 01 laid the **data-model foundation** (the vocabulary needed to represent real songs — `pow`, `Block.label`, schema v7); (2) Phase 02 delivered the **`importSession` translator** (structured chart → Session); (3) the remaining follow-on builds the **user-facing ingestion surface** (import UI + chart sourcing) that consumes the translator. See the design discussion in the Pilot conversation of 2026-07-01/02.
...
**Phase 02** (complete, branch `song-import/phase-02`, **NOT merged — held on branch, no user-facing surface yet**) — `importSession` skill ([`src/agent/import-session.ts`](src/agent/import-session.ts)): a pure translator from a structured song chart → validated `SavedSession`, reusing `pow` + `Block.label` + schema v7. Input contract = **OD-3 Option A** (LLM-native structured chart: `{ songTitle, artist?, bpm, key, mode, sections:[{ label, chords:[{ root, quality, bars? }] }] }`) via `ImportSessionInputSchema` (`IMPORT_SCHEMA_VERSION = 1`), with a `noteToPc` null-guard that rejects invalid note names at the schema boundary. Calls `melodyLine` **directly** (store-free, no `svelte/store`); assembles blocks named `"<songTitle> — <sectionLabel>"` with `label` = bare section, a default `bd` groove, and a single track. Proven by an inlined golden-fixture test (Metallica "ONE", 25 tests). ADR 0026 records the input contract. Decisions OD-3 (input boundary = Option A) and the block-naming convention in `docs/song-import/decisions.md`. 2129 tests. Docs: `docs/song-import/`.

Deferred to Phase 03+: the **UI-wiring surface that makes this user-facing and merge-worthy** — `applyImportSession` in [`src/agent/apply.ts`](src/agent/apply.ts) (load `importSession` output into `sessionStore` → blocks to timeline, harmony to progression) plus an import UI (a "name or link" field that calls the LLM to produce the structured chart, then feeds `importSession`); the **chart-sourcing fork still open by design** (LLM-native generation vs scraping Ultimate Guitar/Chordify); compound/irregular time signatures (7/8, 14/8 — conflicts with the 1-cycle=4/4 invariant, needs its own architecture decision).
