...
- **Phase 10 (scoping)** — Pentagrama as a creation tool ("el pentagrama no es solo visualización, es también construcción"): notes as first-class manipulable elements on the staff — duration-extent rendering (a chord's dots span its sounding time), drag to move in time and in pitch (preserving intervals), per-chord duration resize and delete (✕) on the staff (parity with ProgressionStrip badge editing), rest manipulation on the staff, click-on-line/space to place a note, arpeggio-mode visual (staggered scale-like dots vs three parallel voice lines), and a visible time/bar grid relating staff duration to the rhythm's cycle time. Possible Tonnetz extension: tap a triangle vertex to add a single note instead of a chord.
